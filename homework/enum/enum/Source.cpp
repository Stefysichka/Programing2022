...
HTTPError *promptForErrorCodes(int numberOfErrors);//підказка
...
int main() {
    ...
    bool hasErrorWithCodeToOutput = false;
    ...
    for (int i = 0; i < 5; ++i) {
        if (loggedErrors[i].error == CODE_TO_OUTPUT) {
            ...
            hasErrorWithCodeToOutput = true;
        }
    }
    // print message if no errors with code to output
    if (!hasErrorWithCodeToOutput) {
        cout << "-No userd with this error code" << endl;
    }
    return 0;
}
